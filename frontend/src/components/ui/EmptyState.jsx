...
export default function EmptyState({ icon: Icon = Inbox, title = "Sin registros", subtitle = "No hay datos disponibles para mostrar.", action }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-8 text-center bg-white rounded-xl border" style={{ borderColor: "var(--border)" }}>
      ...
      <p className="text-sm mb-5" style={{ color: "var(--text-muted)", maxWidth: 320 }}>{subtitle}</p>
      {action}
    </div>
  );
}
